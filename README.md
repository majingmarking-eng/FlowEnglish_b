...
![License](https://img.shields.io/badge/license-MIT-green) ![Manifest V3](https://img.shields.io/badge/Chrome-Manifest%20V3-blue) ![version](https://img.shields.io/badge/version-0.3.2-orange)

B 站英文课程学习助手（Chrome / Edge 扩展，Manifest V3）。**零依赖、纯本地运行、不收集任何数据。**

打开 B 站视频页后，**右侧会出现一条 FlowEnglish 侧栏**（独立停靠栏，可折叠为 46px 窄条；B 站内容自动避让，永不重叠）。**手动点击侧栏里的 flowenglish 按钮才激活全部学习功能，不会自动运行**。

> 侧栏顶部有「›」按钮可一键收起为 46px 窄条（B 站内容恢复全宽），再次点击展开。B 站播放器进入全屏时侧栏自动隐藏。

| 侧栏学习面板 | 中译英练习 |
| --- | --- |
| ![侧栏](docs/screenshots/dock.png) | ![练习](docs/screenshots/quiz.png) |
...
- **中译英练习**：基于当前视频字幕本地生成 5 道中译英听写题（不消耗 API）——看中文题面写英文，对照原句自我批改，可一键加入笔记。
...
- 本项目仅供**个人学习交流**使用，请遵守 B 站用户协议及相关法律法规，请勿用于任何商业用途。
- 插件不收集、不上传任何用户数据；ASR / 翻译 / LLM 请求均指向**用户自行配置**的服务商，请自行保管好 API 密钥。
- 本项目与 Bilibili 无任何关联，视频内容的版权归原权利方所有。

## License

[MIT](LICENSE) © 2026 majingmarking-eng
